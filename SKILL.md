...
2. Run `python3 <skill-dir>/ssot_check.py discover --root <target-repo>`.
...
```sh
python3 <skill-dir>/ssot_check.py check --root <target-repo>
python3 <skill-dir>/ssot_check.py explain FACT --root <target-repo>
```

`--json` produces machine-readable output. Exit `0` means tracked facts match,
`1` means drift or staleness, and `2` means a manifest/configuration error.
...
- [Behavioral fixture](tests/fixtures/pointer-audit.md): a fictional cold-start
  audit case; its evaluation rubric is a separate developer resource.
